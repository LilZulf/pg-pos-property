                            ...
                            @foreach ($transactions as $data)
                                <tr class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3"><img
                                            src="{{ $data->channel == 'bri' ? asset('assets/images/bank/bri.png') : ($data->channel == 'mandiri' ? asset('assets/images/bank/mandiri.png') : asset('assets/images/bank/bni.png')) }}"
                                            alt=""></td>
                                    <td
                                        class="px-4 py-3 {{ $data->status == 'success' ? 'text-green-500' : ($data->status == 'pending' ? 'text-yellow-500' : 'text-red-500') }}">
                                        {{ $data->status }}
                                    </td>
                                    <td class="px-4 py-3">{{ $data->amount }}</td>
                                    <td class="px-4 py-3">{{ $data->email }}</td>
                                    <td class="px-4 py-3"> <a class="btn btn-warning mr-4"
                                            href="/transaction/virtual/edit/{{ $data->id }}" role="button"><svg
                                            ...
                                            href="/transaction/virtual/delete/{{ $data->id }}" role="button"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
